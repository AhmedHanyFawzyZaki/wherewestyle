<?php
$this->pageTitle = Yii::app()->name . ' - Followed Products';
?>
...
<?php
Yii::app()->clientScript->registerScript('options_followed_586587', '
        ...
        <div class="row-fluid shops-banners item-page-sale">
            ...
            <div class="row-fluid shops-filterr">
                ...
                <div class="input-append filter"><!--remove - from filter to float right-->
                    ...
                    <input type="hidden" name="type" value="followed" />
                    ...
                    $url = array('home/followedproducts');
                    if($action == "search_products"){
                        $url = array('home/search_products','filter'=>$filter,'query'=>$query);
                    }
                    ?>
                    <?php echo CHtml::link(
                    "<i class='icon-th icon-white'></i>&nbsp; Grid", array_merge($url,array('flag' => 'grid')),
                    array(
                    'class' => 'btn btn-success list',                                              
                    ));?>

                    <?php echo CHtml::link(
                    "<i class='icon-th-list icon-white'></i>&nbsp; List", array_merge($url,array('flag' => 'list')),
                    array(
                    'class' => 'btn btn-success grid',                                              
                    ));?>
                    
                    <?php echo CHtml::link(
                    "<i class='icon-white icon-random'></i>&nbsp; Express", array_merge($url,array('flag' => 'express')),
                    array(
            ...
            <div id="listing">
                <?php $this->renderPartial($view, array('products' => $products, 'sale' => false, 'show_shop' => true)); ?> 
            </div>
            <?php if($view != 'express'){ ?>
            <?php
